...
## 3. Harness como runtime de loops

El Harness se valida porque sus primitivas mapean **literalmente** (no por analogía) a la anatomía del loop. No hay que inventar el motor de loops: ya existe.

| Anatomía del loop | Primitiva Harness | Notas |
|---|---|---|
...
import { defineSchedule } from "harness/schedules";
...
La documentación de Harness es explícita:
...
Esto, lejos de ser un problema, *refuerza* la tesis: Harness te empuja a poner el activo deliberadamente fuera del agente, donde es consultable, versionable y auditable de forma independiente del modelo.

### 3.3 Company Twin como file system (decisión v1)

Para esta iteración: **el Company Twin se hardcodea como file system.** Más adelante migra a una base de datos consultable (Postgres/pgvector). Esto es coherente con la advertencia de Harness (store externo) y mantiene la simplicidad inicial:
...
Requisito no negociable: **cada avance se verifica con evals.** Harness trae un framework de evals completo que convierte "creo que funciona" en "el eval pasa o no pasa". Esto ataca directamente la causa por la que prototipos previos se abandonaron: no había forma objetiva de saber si funcionaban.
...
5. Cada corrida de loop es replayable y deja lineage en el Harness Ledger.
...
El destino no es un set fijo de loops. Es una **plataforma donde el usuario final compone loops y misiones personalizadas** desde una interfaz, todas ancladas al mismo Company Twin (la capa de inteligencia empresarial). Harness lo habilita con `defineDynamic`: tools, skills e instrucciones se resuelven en runtime según la empresa, el usuario y la misión —no se escriben a mano N veces—. La UI emite una *mission spec*; el runtime la materializa contra el Company Twin.

```mermaid
graph TB
    UI[UI: usuario compone loop/misión] -->|mission spec| RT[Harness runtime]
...
## Apéndice — Mapeo Loop Engineering ↔ Sigma ↔ Harness

| Loop Engineering (Osmani) | Sigma (capa) | Harness (primitiva) |
|---|---|---|
...
- [`decisiones.md`](./decisiones.md) — Registro de decisiones arquitectónicas (ADR). La más importante: **Harness es el runtime único** (no DSL+runner+WDK propio).
