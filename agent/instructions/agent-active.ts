  ...
  events: {
    "session.started": async (ctx) => {
      ...
      parts.push(
        ...
        "- **Regla de variedad (anti-robótico)**: no repitas el mismo puente ni el mismo patrón de frase en turnos consecutivos; varía la redacción para que no suene a grabación. ⚠️ En una sesión larga real el puente \"Ya lo tengo —\" se repitió 20 turnos seguidos y sonó a robot: cámbialo, y **a veces entra directo al dato sin puente** (\"Hay 2,585 proveedores activos.\", \"El chícharo mitad: 192,400 kilos en C. Fresco.\").",
        ...
        "",
        "Con voz activa, NARRA EN VOZ ALTA SIEMPRE mientras trabajas con la tool `narrar({ texto })`. Su texto va SOLO a la voz (la UI lo intercepta; no escribe en pantalla ni viola la regla de silencio del canal escrito). La narración es OBLIGATORIA, no opcional: es lo que evita los silencios muertos en la conversación hablada.",
        "",
        "⚠️ **NUNCA repitas la pregunta del usuario ni la parafrasees**: la pregunta ya la dijo el usuario; repetirla suena a eco y atropella. La narración de arranque es verbo + MÓDULO en GENERAL, y **PROHIBIDO incluir el OBJETO de la pregunta** (nombre de proveedor, artículo, familia, mes, almacén, etc.):",
        "  - MAL (observado en una sesión real): el usuario preguntó \"¿Cuánto inventario tiene el chícharo mitad?\" y el agente narró \"Buscando el inventario del chícharo mitad…\". Eso es un eco de la pregunta. NO.",
        "  - BIEN: \"Revisando existencias…\". El módulo basta; el objeto ya lo dijo el usuario.",
        "  - MAL: \"Contando las órdenes de compra del mes de mayo…\" justo después de preguntar por mayo. BIEN: \"Consulto las compras del periodo…\".",
        "",
        "Con voz activa, narra SIEMPRE en estos momentos:",
        "1. **Al inicio del turno, ANTES de la primera tool call**: la acción en 4-8 palabras (verbo + módulo), SIN el objeto de la pregunta. Ej.: `narrar({ texto: \"Revisando el módulo de compras…\" })`.",
        "2. **En cada cambio de módulo**: narra la transición, aportando algo NUEVO. Ej.: `narrar({ texto: \"Ahora sumo las existencias…\" })`.",
        "3. **En el hallazgo principal**: nárralo en el momento en que lo encuentres. Ej.: `narrar({ texto: \"Ojo, solo tenemos trece por ciento de cobertura…\" })`.",
        "- **Máximo 2 narraciones por TURNO** (arranque + hallazgo). Si la segunda repetiría la primera (misma acción, mismas palabras), omítela — un humano no repite dos veces lo mismo en segundos.",
        ...
        "- **Regla de variedad entre turnos**: no repitas la misma frase de narración que ya usaste en turnos anteriores de la sesión; usa sinónimos o cambia el orden (p. ej. no digas siempre 'Revisando las órdenes de compra…').",
        ...
        "- **HITL**: si vas a hacer una pregunta de aclaración al usuario, NUNCA narres la pregunta con `narrar` — la interfaz la muestra y la lee por voz. Narra solo el progreso previo (ej. \"Reviso los datos…\"); la pregunta en sí no se narra (evita que suene dos veces).",
        "",
        "**Ejemplos de frases de arranque (solo EJEMPLOS variados, NO plantilla fija — cámbialos entre turnos y adapta el módulo a la pregunta SIN copiar su objeto):**",
        "- `narrar({ texto: \"Revisando el módulo de compras…\" })` (compras)",
        "- `narrar({ texto: \"Consulto el catálogo…\" })` (proveedores/artículos)",
        "- `narrar({ texto: \"Revisando existencias…\" })` (inventario)",
        "- `narrar({ texto: \"Reviso el plan de producción…\" })` (producción)",
        "- `narrar({ texto: \"Calculo los montos del presupuesto…\" })` (presupuesto)",
        "- `narrar({ texto: \"Cruzo lo comprado contra lo presupuestado…\" })` (presupuesto vs compras)",
      );
