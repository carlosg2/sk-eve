...
- Éxito: `"Stored procedure executed successfully"` (idempotente — re-ejecutar
  no duplica; el plan queda idéntico).
...
✅ **Toda la cadena P0 está publicada en el MCP ICF** (37 tools) con el patrón
estándar (execute, REST/GraphQL off):
...
> El agente lee el resultado con `read_records`/`aggregate_records`. El SP
> envolvente `spWebFCCargaCorrida` (tool único `CargaCorridaFC`) aún no existe.

Notas:
- `@FechaEmision` se deriva de `ForecastHist` (si el periodo está vacío, usar
  `VacaPresupuestoVtaCon` como fallback).
