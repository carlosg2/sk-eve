<script lang="ts">
	...
	onMount(() => {
		// Handle canvas resizing. Guard de cambio: si el bitmap no cambió, no
		// mutamos nada — escribir style.width/height inline a un canvas `w-full
		// h-full` dentro del container observado re-disparaba el ResizeObserver
		// en el mismo frame ("ResizeObserver loop completed with undelivered
		// notifications"). El tamaño del BITMAP (atributos) es lo único que hace
		// falta; el CSS controla el layout del canvas.
		const resizeObserver = new ResizeObserver(() => {
			...
			if (canvasRef.width === w && canvasRef.height === h) return;
			canvasRef.width = w;
			canvasRef.height = h;
			const ctx = canvasRef.getContext("2d");
			if (ctx) {
				// setTransform (no scale acumulativo): el contexto se resetea al
				// cambiar width/height, pero setTransform es idempotente.
