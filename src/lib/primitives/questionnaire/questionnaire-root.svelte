<script lang="ts">
	...
	type Props = Omit<HTMLFormAttributes, "children" | "item" | "onsubmit" | "onreset"> & {
		...
		// Enfoca el primer item al montar. Desactivar (false) cuando el form vive
		// DENTRO de un transcript (HITL gate): el focus nativo arrastra el scroll
		// del contenedor hacia el item y rompe el scroll controlado del chat.
		autofocus?: boolean;
	...
	function setItem(name: string, focusTarget: "item" | "invalid" = "item") {
		...
		// El foco se omite con autofocus=false: el focus nativo del navegador hace
		// scroll hacia el elemento y rompería el scroll controlado del transcript.
		if (autofocus) void focus(name, focusTarget);
	}
