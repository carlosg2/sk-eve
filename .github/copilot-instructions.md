...
- Endpoints: `GET /api/audit/turns` (radiografía por turno: pregunta + respuesta + tools + tokens + errores), `GET /api/audit/llm` (inputs), `GET /api/audit/reasoning` (razonamiento reconstruido por turno), `GET /api/traces` (resúmenes + tendencia), `GET /api/sessions`.
