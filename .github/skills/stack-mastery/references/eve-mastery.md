...
- **⚠️ `eve dev` en background "secuestra" los arranques posteriores de `npm run dev`** (validado 2026-08-10): si un `eve dev` quedó corriendo (p.ej. en `127.0.0.1:2000`), el siguiente `npm run dev` imprime "A dev server is already running for this eve agent" y NO publica URL nueva → el plugin de vite espera 120s y falla con "Timed out waiting for eve to print its server URL". **No es un bug del entorno** — es un proceso huérfano del diagnóstico. Verificar con `lsof -iTCP:2000` y matar el `eve dev` antes de reintentar. Nunca dejar `eve dev` en background si luego se va a correr `npm run dev`.
