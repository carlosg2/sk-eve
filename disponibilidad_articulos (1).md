...
SELECT
  ...
  ISNULL(VentaD.Precio, 0.00) * ISNULL(VentaD.Cantidad, 0.00) AS ImporteDetalle
FROM Venta
JOIN VentaD ON Venta.ID = VentaD.ID
JOIN Cte ON Venta.Cliente = Cte.Cliente
JOIN Art ON VentaD.Articulo = Art.Articulo
JOIN MovTipo ON Venta.Mov = MovTipo.Mov AND MovTipo.Modulo = 'VTAS'
WHERE Venta.Estatus = 'PENDIENTE'
AND Movtipo.Clave = 'VTAS.P'
ORDER BY
  Venta.Ejercicio,
